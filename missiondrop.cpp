...
void visionClass::missionDrop()
{
    ...
    //任务变量
    Mat m2_imgsrc = cv::Mat::zeros(580, 780, CV_8UC3),					//源图像
        m2_imgblur= cv::Mat::zeros(580, 780, CV_8UC3),					//中值滤波图像
        // m2_imghsv,					//HSV颜色空间
        // m2_bgr_streched,			//双通道拉伸之后的RGB图像
        m2_imgranged= cv::Mat::zeros(580, 780, CV_8UC3),				//双边阈值颜色识别之后的图像
        // m2_imgcanny,				//边缘识别之后的图像
        m2_imgresult= cv::Mat::zeros(580, 780, CV_8UC3);
    ...
    static const float m2_imgscale = 1;			//图像缩放比例
    ...
    colorThresh.RV_min = 30;
    colorThresh.RB_max = 24;
    colorThresh.RH_max = 80;
    colorThresh.RV_max = 96;
    ...
    tDrum_hand_range range;
    range.drum_low = Scalar(0, 0, 0);
    range.drum_high = Scalar(42, 48, 108);
    range.hand_low = Scalar(78, 39, 84);
    range.hand_high = Scalar(255, 230, 255);
...
//    emit createBar( "V_ranged", "Color", &colorThresh.RV_min, 255);
    ...
    while(m2_start)
    {
        // visionClass::start(Cam_Bottom); // Bottom is OK !!! but cannot use on Front !!!
        expotime_d++;
        if(expotime_d<10)
              DropCamSetting();
...
//        }
        ...
        m2_imgsrc = autocontrost(m2_imgsrc);

        result = m2_imgsrc.clone();
        if (get_hand_in_drum(result, hand, range, drum)) {
            putText(result, "Hand In", Point(result.cols*0.1, result.rows*0.2), FONT_HERSHEY_SIMPLEX, 1, Scalar(0,0,255), 2, 8);
            data.m2_hand_in = 1;
        } else {
            data.m2_hand_in = 0;
        }

        emit imageShow(&result, "m2_imgsrc");
        ...
        if( m2_Rect.valid == true)
        {
            ...
            data.m2_drum_dy = (0.5*m2_imgsrc.rows - 0.5*( m2_Rect.tl_y + m2_Rect.br_y)) / m2_imgscale;
        }
        //如果本次矩形拟合无效，给999
        else
        {
...
//        qDebug() << "m2_drum_dy:" << data.m2_drum_dy;
    ...
    m2_imgblur.release();
    m2_imgranged.release();
    m2_imgresult.release();
    m2_imgsrc.release();
